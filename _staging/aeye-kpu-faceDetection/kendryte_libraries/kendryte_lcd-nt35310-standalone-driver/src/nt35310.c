...
void tft_set_clk_freq(uint32_t freq) { spi_set_clk_rate(SPI_CHANNEL, freq); }
...
void tft_hard_init(uint32_t freq, bool oct)
{
    ...
    else
    {
        ...
        sysctl_set_spi0_dvp_data(1); // important!
...
void tft_write_command(uint8_t cmd)
{
    ...
    if (!standard_spi)
    {
        ...
        spi_send_data_normal_dma(SPI_DMA_CH, SPI_CHANNEL, LCD_SPI_SLAVE_SELECT,
...
void tft_write_half(uint16_t *data_buf, uint32_t length)
{
    ...
    if (!standard_spi)
    {
        ...
        spi_init_non_standard(SPI_CHANNEL, 0 /*instrction length*/,
                              16 /*address length*/, 0 /*wait cycles*/,
                              SPI_AITM_AS_FRAME_FORMAT /*spi address trans mode*/);
        spi_send_data_normal_dma(SPI_DMA_CH, SPI_CHANNEL, LCD_SPI_SLAVE_SELECT,
...
void tft_fill_data(uint32_t *data_buf, uint32_t length)
{
    ...
    if (!standard_spi)
    {
        ...
        spi_init_non_standard(SPI_CHANNEL, 0 /*instrction length*/,
                              32 /*address length*/, 0 /*wait cycles*/,
                              SPI_AITM_AS_FRAME_FORMAT /*spi address trans mode*/);
        spi_fill_data_dma(SPI_DMA_CH, SPI_CHANNEL, LCD_SPI_SLAVE_SELECT, data_buf,
                          length);
    }
    else
    {
