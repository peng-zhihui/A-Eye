...
void Camera::switch_gram()
{
    Camera::g_ram_mux ^= 0x01;
    Camera::g_dvp_finish_flag = 0;
}

bool Camera::grab_frame_done()
{
    return Camera::g_dvp_finish_flag;
}
...
int on_irq_dvp(void *ctx)
{
    ...
    {
        /* switch gram */
        dvp_set_display_addr(Camera::g_ram_mux ? (long)Camera::g_lcd_gram0 : (long)Camera::g_lcd_gram1);
